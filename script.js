...
function showKav(idx,btn){
  ...
  document.querySelectorAll('.kav')
    .forEach(k=>k.classList.remove('show'));

  document.querySelectorAll('.ptab')
    .forEach(b=>b.classList.remove('active'));

  document.getElementById('kav'+idx)
    .classList.add('show');

  btn.classList.add('active');

  for(let i=0;i<5;i++){
    setTimeout(spawnP,i*100);
  }
}
